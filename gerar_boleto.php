...
require 'config_opcoes.php'; //configurar opções de grau dependencia
...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Verifica se os dados foram enviados via método POST
    ...
    if ($valid) {
        // Prepara a consulta para inserir os dados do novo boleto pago no banco de dados
        $sql_insert = "INSERT INTO BoletosPagos (cpf, matricula, vinculo, nome, grau_dependencia, data_pagamento, valor_pago, tipo_registro, data_vencimento, referencia_pagamento, situacao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt_insert = $conn->prepare($sql_insert);
        $stmt_insert->bind_param("ssssssdssss", $cpf, $matricula, $vinculo, $nome, $grau_dependencia, $data_pagamento, $valor_pago, $tipo_registro, $data_vencimento, $referencia_pagamento, $situacao);
        ...
        if ($stmt_insert->execute()) {
            ...
            echo "Erro ao gerar o boleto: " . $conn->error;
        }
    ...
    $cpf = "12345678901"; //Valores padrão para exibição inicial
    $matricula = "001";
    $vinculo = "1";
    $nome = "SeuNome";
    $grau_dependencia = "0";
    ...
    $referencia_pagamento = "2000-01-01";
    $situacao = 1;
}
?>
...
<div class="container mt-5">
    ...
    <form action="" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" name="data_pagamento" value="<?= htmlspecialchars($data_pagamento ?? '', ENT_QUOTES); ?>" class="form-control" required>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="data_vencimento" value="<?= htmlspecialchars($data_vencimento ?? '', ENT_QUOTES); ?>" class="form-control" required>
        </div>
        <div class="mb-3">
            <label>Referência de Pagamento</label>
            <input type="date" name="referencia_pagamento" value="<?= htmlspecialchars($referencia_pagamento ?? '', ENT_QUOTES); ?>" class="form-control" required>
        </div>
        <div class="mb-3 form-check">
            <input type="checkbox" name="situacao" value="1" class="form-check-input" id="situacao">
